import {
    ...
    findVideoByUserQuery,
    insertStatsQueryString,
    updateStatsQueryString,
    watchedVideosQueryString,
    favouritedVideosQueryString,
} from "./graphql";
...
export const fetchFavouritedVideos = async (user_id, token) => {
    try {
        const data = await queryHasuraGraphQL(favouritedVideosQueryString, `query_favourited_videos_by_user`, {user_id}, token)
        return data?.stats;
    } catch (error) {
        console.log(`Fetching favourited videos failed! `, error);
    }
}
